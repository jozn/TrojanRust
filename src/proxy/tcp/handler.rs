...
// use crate::config::tls::{make_client_config_dep, NoCertificateVerification_Dep};
...
impl TcpHandler {
    ...
    pub fn init(outbound: &OutboundConfig_Dep) -> &'static TcpHandler {
        ...
        //     Some(cfg) => {
        //         let client_config = make_client_config_dep(&cfg);
        //         Some((
        //             client_config,
        ...
        let secret = match outbound.protocol {
            SupportedProtocols_Dep::TROJAN if outbound.secret.is_some() => {
    ...
    async fn handle_direct_stream<T: AsyncRead + AsyncWrite + Unpin + Send>(
        ...
        let (proxy_protocol, transport_protocol) =
            (request.proxy_protocol_dep, request.transport_protocol);
